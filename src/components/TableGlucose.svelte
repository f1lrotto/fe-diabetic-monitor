<script>
  ...
  $: readingsByDay = sortedData.reduce((acc, item) => {
    ...
    if (!acc[dateKey].some(reading => reading.timestamp === item.timestamp) && ($filterOption ? [0, 15, 30, 45] : [0, 30]).includes(new Date(item.timestamp).getMinutes())) {
...
<style>
  ...
  .high { background-color: #de2c2c; }
  .medium { background-color: #faa302; }

  button {
    color: #555; /* Similar to .unitSwitch */
    text-decoration: none;
    padding: 10px 15px;
    margin-right: 10px; /* Space between items */
    margin-top: 10px;
    border: 1px solid rgba(0, 0, 0, 0.2); /* Border for boxy look */
      ...
      <div class="day-container">
        ...
        {#each readingsByDay[day] as item (item._id)}
          <div class="reading">
            ...
            <div class="glucose-level {item.glucoseMMOL > 10 || item.glucoseMMOL < 3.5 ? 'high' : item.glucoseMMOL > 8 || item.glucoseMMOL < 4 ? 'medium' : ''}">
